...
public class RobotMap {
	...
	public static void init() {
		...
		Trajectory.Config config = new Trajectory.Config(Trajectory.FitMethod.HERMITE_QUINTIC, Trajectory.Config.SAMPLES_HIGH, 0.02, 8.5, 6.0, 1000000.0);

		Waypoint[] points0 = new Waypoint[] {
				new Waypoint(-4, -1, Pathfinder.d2r(0.785)),      // Waypoint @ x=-4, y=-1, exit angle=-45 degrees
	//			new Waypoint(-2, -2, 0),                        // Waypoint @ x=-2, y=-2, exit angle=0 radians
				new Waypoint(0, 0, 0)                           // Waypoint @ x=0, y=0,   exit angle=0 radians
		};

		Trajectory.Config config2 = new Trajectory.Config(Trajectory.FitMethod.HERMITE_CUBIC, Trajectory.Config.SAMPLES_HIGH, 0.05, 6.0, 3.0, 100000.0);
		drive0 = Pathfinder.generate(points0, config2);


		File drive0file = new File("/home/lvuser/Trajectories/drive0.t");
		try {
			drive0file.createNewFile();
			Pathfinder.writeToFile(drive0file, drive0);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		drive0 = Pathfinder.readFromFile(new File("/home/lvuser/Trajectories/drive0.t"));
				...
				drive1 = Pathfinder.generate(points1, config2);
